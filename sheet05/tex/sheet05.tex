...
\begin{enumerate}[a)]
    ...
    \item 
        ...
        We are achieving a small error decrease.
        \begin{figure}[H]
            \centering
        ...
        \end{figure}
        Our plots show that, just as expected, the left-right mirroring increases the error-rate on the training data quite a lot (traintop1 from basically 0\% to 30\% error) because the data we are using in every epoch to train the network is not equal to the training set itself and is changing every time (because 50\% of the pictures randomly chosen are flipped). This also has the effect to reduce overfitting significantly, which results in an overall smaller error on the test-set (65\% using the left-right mirroring and 70\% without).
